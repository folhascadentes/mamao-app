import { HandShapeType } from "./types";

export const HandshapeImages: {
  [key: HandShapeType]: { label: string; path: string; alt?: string }[];
} = {
  a: [
    {
      label: "Visão frontal",
      path: "/handshapes/a_front.png",
    },
    {
      label: "Visão lateral",
      path: "/handshapes/a_side.png",
    },
  ],
  d: [
    {
      label: "Visão frontal",
      path: "/handshapes/d_front.png",
    },
    {
      label: "Visão lateral",
      path: "/handshapes/d_side.png",
    },
  ],
  f: [
    {
      label: "Visão frontal",
      path: "/handshapes/f_front.png",
    },
    {
      label: "Visão lateral",
      path: "/handshapes/f_side.png",
    },
  ],
  l: [
    {
      label: "Visão frontal",
      path: "/handshapes/l_front.png",
    },
    {
      label: "Visão lateral",
      path: "/handshapes/l_side.png",
    },
  ],
  o: [
    {
      label: "Visão frontal",
      path: "/handshapes/o_front.png",
    },
    {
      label: "Visão lateral",
      path: "/handshapes/o_side.png",
    },
  ],
  s: [
    {
      label: "Visão frontal",
      path: "/handshapes/s_front.png",
    },
    {
      label: "Visão lateral",
      path: "/handshapes/s_side.png",
    },
  ],
  y: [
    {
      label: "Visão frontal",
      path: "/handshapes/y_front.png",
    },
    {
      label: "Visão lateral",
      path: "/handshapes/y_side.png",
    },
  ],
  claw: [
    {
      label: "Visão frontal",
      path: "/handshapes/claw_front.png",
    },
    {
      label: "Visão lateral",
      path: "/handshapes/claw_side.png",
    },
  ],
  indexFinger: [
    {
      label: "Visão frontal",
      path: "/handshapes/index_finger_front.png",
    },
    {
      label: "Visão lateral",
      path: "/handshapes/index_finger_side.png",
    },
  ],
  middleAndIndexFinger: [
    {
      label: "Visão frontal",
      path: "/handshapes/middle_index_finger_front.png",
    },
    {
      label: "Visão lateral",
      path: "/handshapes/middle_index_finger_side.png",
    },
  ],
  middleFingerBendedFingersApart: [
    {
      label: "Visão frontal",
      path: "/handshapes/middle_finger_bended_fingers_apart_front.png",
    },
    {
      label: "Visão lateral",
      path: "/handshapes/middle_finger_bended_fingers_apart_side.png",
    },
  ],
  oi: [
    {
      label: "Visão frontal",
      path: "/handshapes/oi_front.png",
    },
    {
      label: "Visão lateral",
      path: "/handshapes/oi_side.png",
    },
  ],
  openHand: [
    {
      label: "Visão frontal",
      path: "/handshapes/open_hand_front.png",
    },
    {
      label: "Visão lateral",
      path: "/handshapes/open_hand_side.png",
    },
  ],
  openHandFingersApart: [
    {
      label: "Visão frontal",
      path: "/handshapes/open_hand_fingers_apart_front.png",
    },
    {
      label: "Visão lateral",
      path: "/handshapes/open_hand_fingers_apart_side.png",
    },
  ],
  openHandThumbApart: [
    {
      label: "Visão frontal",
      path: "/handshapes/open_hand_thumb_apart_front.png",
    },
    {
      label: "Visão lateral",
      path: "/handshapes/open_hand_thumb_apart_side.png",
    },
  ],
  thumbFinger: [
    {
      label: "Visão frontal",
      path: "/handshapes/thumbs_up_front.png",
    },
  ],
  thumbTouchIndexFingersOpen: [
    {
      label: "Visão frontal",
      path: "/handshapes/thumb_touch_index_fingers_open_front.png",
    },
    {
      label: "Visão lateral",
      path: "/handshapes/thumb_touch_index_fingers_open_side.png",
    },
  ],
  thumbTouchIndexFingersClosed: [
    {
      label: "Visão frontal",
      path: "/handshapes/thumb_touch_index_fingers_closed_front.png",
    },
    {
      label: "Visão lateral",
      path: "/handshapes/thumb_touch_index_fingers_closed_side.png",
    },
  ],
};
